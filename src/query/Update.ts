...
/** Type for UPDATE SET values — all columns optional, values match column types */
export type InferUpdate<T extends TableDefinition> = Partial<{
  [K in keyof T["columns"]]: InferColumnType<T["columns"][K]> | null
}>
...
> {
  ...
  set(
    values: TTable extends TableDefinition ? InferUpdate<TTable> : Record<string, unknown>
  ): UpdateBuilder<TTable, TResult> {
    const b = this._clone()
    b._set = { ...this._set, ...(values as Record<string, unknown>) }
    return b
  }

  where(...conditions: WhereCondition[]): UpdateBuilder<TTable, TResult> {
    const b = this._clone()
  ...
  with(...ctes: CteClause[]): UpdateBuilder<TTable, TResult> {
    const b = this._clone()
  ...
  returning(): UpdateBuilder<TTable, TTable extends TableDefinition ? InferSelect<TTable> : Record<string, unknown>>
  // Overload: selection map → typed result
  returning<TSelection extends Record<string, ColumnOrExpr>>(
    selection: TSelection
  ): UpdateBuilder<TTable, SelectionResult<TSelection>>
  // Overload: specific columns (backward compat)
  returning(...columns: Array<ColumnDef<any, any, any> | string>): UpdateBuilder<TTable, TResult>
  // Implementation
  returning(...args: any[]): UpdateBuilder<TTable, any> {
    const b = this._clone() as UpdateBuilder<TTable, any>
    if (args.length === 0) {
      b._returning = ["*"]
  ...
  toSql(): Statement {
    ...
    const setClauses = Object.entries(this._set).map(([key, value]) => {
      if (value instanceof Expression) {
        const exprSql = resolvePlaceholders(value.sql, value.params)
        return `"${key}" = ${exprSql}`
      }
      params.push(value)
      return `"${key}" = $${paramIdx++}`
  ...
  private _clone(): UpdateBuilder<TTable, TResult> {
    const b = new UpdateBuilder<TTable, TResult>(this._table as any)
